...
function SoloGame(callback) {
    ...
    const moveComputer = () => {
        if (state.right - 13 > state.y || state.right + 114 < state.y)
            this.moveRightPaddle(state.right - 13 > state.y ? 'ArrowDown' : 'ArrowUp')
    };
    
    this.start = () => {
        state.intervals.push(setInterval(moveBall,40));
        state.computerInterval = setInterval(moveComputer,100)
    }
